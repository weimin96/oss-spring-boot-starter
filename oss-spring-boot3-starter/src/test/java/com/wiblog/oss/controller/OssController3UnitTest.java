 ...
 * OssController3 单元测试。
 *
 * <p>现有集成测试已经覆盖了真实 MinIO 链路，这里补控制器级单元测试，
 * 目的是把薄控制器的委派分支、参数转换和返回包装补齐，避免覆盖率被简单转发方法拉低。</p>
 */
@DisplayName("OssController3 单元测试")
class OssController3UnitTest {
    ...
    private OssController3 controller;
    ...
    void setUp() {
        ...
        ossProperties = new OssProperties("http://localhost:9000", "ak", "sk", "minio", "unit-bucket");
        ...
        when(queryOperations.getOssProperties()).thenReturn(ossProperties);

        controller = new OssController3(ossTemplate);
    }
    ...
    class UploadApiTest {
        ...
        void multipartEndpointsDelegateToPutOperations() {
            ...
            List<Part> parts = Collections.singletonList(Part.builder().partNumber(1).build());

            when(putOperations.initTask(chunkTask)).thenReturn("upload-id");
            when(putOperations.chunk(chunk)).thenReturn(chunkTarget);
            ...
            assertThat(controller.chunk(chunk).getData()).isSameAs(chunkTarget);
            ...
            verify(putOperations).chunk(chunk);
    ...
    class QueryApiTest {
        ...
        void deleteEndpointsDelegate() {
            List<String> objectNames = List.of("a.txt", "b.txt");
        ...
        void queryEndpointsDelegate() {
            ...
            List<ObjectInfo> objectInfos = List.of(objectInfo);
            List<ObjectTreeNode> treeNodes = List.of(folderNode);
            ...
            when(queryOperations.getAllBuckets()).thenReturn(List.of(BucketInfo.builder().name("unit-bucket").build()));
    ...
    class PreviewAndFileOperationTest {
        ...
        void copyMoveAndPresignDelegate() {
            when(presignOperations.generateGetPresignedUrl("demo/file.txt", java.time.Duration.ofSeconds(60)))
                    .thenReturn("get-url");
            when(presignOperations.generatePutPresignedUrl(
                    "demo/file.txt", "text/plain", java.time.Duration.ofSeconds(120), null))
                    .thenReturn("put-url");
            ...
            verify(presignOperations).generateGetPresignedUrl("demo/file.txt", java.time.Duration.ofSeconds(60));
    ...
    class UnzipAndTagTest {
        ...
        void unzipEndpointsDelegate() {
            UnzipResult unzipResult = UnzipResult.builder()
                    .succeeded(List.of(ObjectInfo.builder().name("a.txt").uri("demo/unzip/a.txt").build()))
        ...
        void taggingEndpointsDelegate() {
            Map<String, String> objectTags = Map.of("env", "test");
            Map<String, String> bucketTags = Map.of("owner", "team");
    ...
    class BucketApiTest {
        ...
        void bucketResourceEndpointsDelegate() {
            ...
            BucketAccessInfo accessInfo = BucketAccessInfo.builder().bucketName("unit-bucket").acl("private").supported(true).build();
            BucketRewindResult rewindResult = BucketRewindResult.builder().bucketName("unit-bucket").targetTime("2026-04-17T12:00:00Z").build();
        ...
        void bucketAdminEndpointsDelegate() {
            ...
            when(bucketOperations.getLifecycleRules()).thenReturn(List.of(lifecycleRuleInfo));
            when(bucketOperations.getCorsRules()).thenReturn(List.of(corsRuleInfo));
