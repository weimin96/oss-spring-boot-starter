...
class OssControllerIntegrationTest extends AbstractControllerDynamicPropertyTest {
    ...
    class ObjectApiTest {
        ...
        void uploadAndQueryObject() throws Exception {
            ...
            mockMvc.perform(get("/oss/object/getObject")
    ...
    class ChunkApiTest {
        ...
        void initTask() throws Exception {
            mockMvc.perform(post("/oss/initTask")
        ...
        void chunkUpload() throws Exception {
            ...
            mockMvc.perform(multipart("/oss/chunk")
                            .file(chunkFile)
                            .param("chunkNumber", "1")
    ...
    class SupportObjectTest {
        ...
        @Test
        @DisplayName("全局异常处理器应保留业务错误码")
        void exceptionHandler() {
            OssGlobalExceptionHandler handler = new OssGlobalExceptionHandler();

            R<Void> result = handler.handleOssException(OssException.bucketNotFound("test-bucket"));

            assertThat(result.isSuccess()).isFalse();
            assertThat(result.getMsg()).contains("BUCKET_NOT_FOUND");
        }

        @Test
        @DisplayName("R.data(null) 应返回无数据提示")
        void responseWrapper() {
            R<String> result = R.data(null);
