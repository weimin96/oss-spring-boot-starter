...
- 单文件上传
- 创建文件夹
- 分片上传；合并请求必须携带预期分片数和文件总大小，服务端会回查并校验真实分片
...
- 下载文件
- 分片下载（HTTP Range）
- Java API 类型化受限流式读取（`offset + length`）
...
- 复制文件，小对象使用单次服务端复制，大对象自动切换为分片服务端复制
- 移动文件；源对象删除使用 ETag 条件，源在移动期间变化时会保留源并报错
...
- Bucket ACL
- 版本控制
- Bucket 时间回滚；仅支持 `Enabled` 版本控制状态，大对象历史版本自动分片复制
- 生命周期
- CORS 配置
- 访问策略
- 安全配置
