...
<template>
  <div>
    ...
    <p class="text-sm text-[var(--color-muted)] mb-5">同 Bucket 内文件复制与移动（移动 = 复制 + 删除源）</p>
    ...
    <ApiCard method="POST" path="/oss/object/copy" summary="复制文件（同 Bucket 内）">
      ...
      <ResultPanel :status="copyStatus" :result="copyResult" :error="copyError" label="复制结果" />
    </ApiCard>
    ...
    <ApiCard method="POST" path="/oss/object/move" summary="移动文件（复制后删除源文件）">
      ...
      <ResultPanel :status="moveStatus" :result="moveResult" :error="moveError" label="移动结果" />
    </ApiCard>
  </div>
</template>
