  ...
  <div>
    ...
    <ApiCard method="GET" path="/oss/object" summary="获取文件详情（元数据）">
      ...
      <button class="btn btn-ghost" :disabled="getStatus === 'loading'"
              @click="execGet(() => ossApi.query.getObject(objectNameGet))">
        <span v-if="getStatus === 'loading'" class="spinner"/>查询
      </button>
      <div v-if="getIsEmpty"
           class="mt-3 p-2 rounded border border-[var(--color-warning)] bg-[rgba(210,153,34,0.08)] text-[var(--color-warning)] text-xs">
        没有数据
      </div>
