...
<template>
  <div>
    ...
    <p class="text-sm text-[var(--color-muted)] mb-5">创建 Bucket、版本控制、生命周期、CORS、访问策略、安全配置</p>

    <!-- 创建 Bucket -->
    <ApiCard method="POST" path="/oss/bucket" summary="创建 Bucket">
      <div class="mb-3">
        <p class="section-label">bucketName</p>
        <input v-model="newBucketName" class="oss-input" placeholder="my-new-bucket" />
      </div>
      <button class="btn btn-primary" :disabled="createStatus === 'loading'" @click="execCreate(() => ossApi.bucket.create(newBucketName))">
        <span v-if="createStatus === 'loading'" class="spinner" />创建 Bucket
      </button>
      <ResultPanel :status="createStatus" :result="createResult" :error="createError" />
    </ApiCard>

    <!-- 版本控制 -->
    <div class="card mb-4">
      <div class="flex items-center gap-3 mb-4">
        <p class="text-sm font-medium">版本控制（Versioning）</p>
      </div>
      <div class="flex gap-2 flex-wrap mb-2">
        <button class="btn btn-ghost" :disabled="getVerStatus === 'loading'" @click="execGetVer(() => ossApi.bucket.getVersioning())">
          <span v-if="getVerStatus === 'loading'" class="spinner" />GET 查询状态
        </button>
        <button class="btn btn-success" :disabled="enableVerStatus === 'loading'" @click="execEnableVer(() => ossApi.bucket.enableVersioning())">
          <span v-if="enableVerStatus === 'loading'" class="spinner" />PUT 启用
        </button>
        <button class="btn btn-ghost" :disabled="suspendVerStatus === 'loading'" @click="execSuspendVer(() => ossApi.bucket.suspendVersioning())">
          <span v-if="suspendVerStatus === 'loading'" class="spinner" />PUT 挂起
        </button>
      </div>
      <ResultPanel :status="getVerStatus" :result="getVerResult" :error="getVerError" label="版本控制状态" />
      <ResultPanel :status="enableVerStatus" :result="enableVerResult" :error="enableVerError" />
      <ResultPanel :status="suspendVerStatus" :result="suspendVerResult" :error="suspendVerError" />
    </div>
    ...
    <div class="card mb-4">
      ...
      <div class="flex gap-2 mb-4">
        <button class="btn btn-ghost" :disabled="getLifeStatus === 'loading'" @click="execGetLife(() => ossApi.bucket.getLifecycle())">
          <span v-if="getLifeStatus === 'loading'" class="spinner" />GET 查询规则
        </button>
        <button class="btn btn-danger" :disabled="delLifeStatus === 'loading'" @click="execDelLife(() => ossApi.bucket.deleteLifecycle())">
          <span v-if="delLifeStatus === 'loading'" class="spinner" />DELETE 删除所有规则
        </button>
      </div>
      <ResultPanel :status="getLifeStatus" :result="getLifeResult" :error="getLifeError" label="生命周期规则列表" />
      <ResultPanel :status="delLifeStatus" :result="delLifeResult" :error="delLifeError" />
      ...
      <div class="border-t border-[var(--color-border)] pt-4 mt-2">
        ...
        <div class="grid grid-cols-3 gap-3 mb-3">
          ...
          <div>
            <p class="section-label">expirationDays</p>
            <input v-model.number="lifecycleDays" type="number" class="oss-input" min="1" />
          </div>
        </div>
        <button
          class="btn btn-ghost"
          :disabled="addLifeStatus === 'loading'"
        ...
        <ResultPanel :status="addLifeStatus" :result="addLifeResult" :error="addLifeError" />
      </div>
    </div>
    ...
    <div class="card mb-4">
      ...
      <div class="flex gap-2 mb-2 flex-wrap">
        <button class="btn btn-ghost" :disabled="getCorsStatus === 'loading'" @click="execGetCors(() => ossApi.bucket.getCors())">
          <span v-if="getCorsStatus === 'loading'" class="spinner" />GET 查询 CORS
        </button>
        <button class="btn btn-primary" :disabled="allowCorsStatus === 'loading'" @click="execAllowCors(() => ossApi.bucket.allowAllCors())">
          <span v-if="allowCorsStatus === 'loading'" class="spinner" />PUT 允许所有来源
        </button>
        <button class="btn btn-danger" :disabled="delCorsStatus === 'loading'" @click="execDelCors(() => ossApi.bucket.deleteCors())">
      ...
      <ResultPanel :status="allowCorsStatus" :result="allowCorsResult" :error="allowCorsError" />
      <ResultPanel :status="delCorsStatus" :result="delCorsResult" :error="delCorsError" />
    </div>

    <!-- Policy -->
    <div class="card mb-4">
      <p class="text-sm font-medium mb-4">访问策略（Bucket Policy）</p>
      <button class="btn btn-ghost mb-3" :disabled="getPolicyStatus === 'loading'" @click="execGetPolicy(() => ossApi.bucket.getPolicy())">
      ...
      <div class="border-t border-[var(--color-border)] pt-4 mt-3">
        ...
        <textarea v-model="policyJson" class="oss-input mb-3" rows="8" style="resize: vertical; font-size: 12px;" />
        <div class="flex gap-2">
          <button class="btn btn-primary" :disabled="putPolicyStatus === 'loading'" @click="execPutPolicy(() => ossApi.bucket.putPolicy(policyJson))">
            <span v-if="putPolicyStatus === 'loading'" class="spinner" />PUT 设置策略
          </button>
          <button class="btn btn-danger" :disabled="delPolicyStatus === 'loading'" @click="execDelPolicy(() => ossApi.bucket.deletePolicy())">
            <span v-if="delPolicyStatus === 'loading'" class="spinner" />DELETE 删除策略
          </button>
        </div>
        <ResultPanel :status="putPolicyStatus" :result="putPolicyResult" :error="putPolicyError" />
        <ResultPanel :status="delPolicyStatus" :result="delPolicyResult" :error="delPolicyError" />
      </div>
    </div>
    ...
    <div class="card mb-4">
      ...
      <p class="text-xs text-[var(--color-muted)] mb-4">这些操作会修改 Bucket 的安全设置，请在了解影响后执行。</p>
      ...
      <div class="flex gap-2 flex-wrap">
        <button class="btn btn-ghost" :disabled="encStatus === 'loading'" @click="execEnc(() => ossApi.bucket.enableEncryption())">
          <span v-if="encStatus === 'loading'" class="spinner" />启用服务端加密（SSE-S3）
        </button>
        <button class="btn btn-danger" :disabled="blockStatus === 'loading'" @click="execBlock(() => ossApi.bucket.blockPublicAccess())">
          <span v-if="blockStatus === 'loading'" class="spinner" />屏蔽所有公共访问
        </button>
      </div>
      <ResultPanel :status="encStatus" :result="encResult" :error="encError" />
      <ResultPanel :status="blockStatus" :result="blockResult" :error="blockError" />
    </div>
  </div>
</template>
