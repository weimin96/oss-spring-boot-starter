...
function addBucketRow() {
  bucketTagRows.push({key: '', value: ''})
}

function removeBucketRow(i: number) {
  bucketTagRows.splice(i, 1)
}
...
const {
  status: getBucketTagStatus,
  result: getBucketTagResult,
  error: getBucketTagError,
  execute: execGetBucketTag
} = useResult()
const {
  status: setBucketTagStatus,
  result: setBucketTagResult,
  error: setBucketTagError,
  execute: execSetBucketTag
} = useResult()
const {
  status: delBucketTagStatus,
  result: delBucketTagResult,
  error: delBucketTagError,
  execute: execDelBucketTag
} = useResult()
</script>
...
<template>
  <div>
    ...
    <ApiCard method="GET" path="/oss/bucket/tags" summary="获取 Bucket 标签">
      ...
      <ResultPanel :status="getBucketTagStatus" :result="getBucketTagResult" :error="getBucketTagError"
                   label="Bucket 标签 Map"/>
    </ApiCard>
