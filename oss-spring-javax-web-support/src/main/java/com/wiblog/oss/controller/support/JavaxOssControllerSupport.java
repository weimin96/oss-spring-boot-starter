...
/**
 * `javax.servlet` 体系共享的 OSS HTTP 端点实现。
 *
 * <p>该基类只承载 HTTP 参数适配与业务委派逻辑，
 * 不直接声明 `@RestController`、`@Validated` 或类级路由。
 * 这样可以把运行时 Bean 注册责任继续留在版本 Starter 中，
 * 同时允许 Boot2 的公开控制器类型保持稳定。</p>
 *
 * @author panwm
 */
@RequiredArgsConstructor
public abstract class JavaxOssControllerSupport implements OssHttpEndpoint {
    ...
    public void previewObject(HttpServletResponse response, HttpServletRequest request)
        ...
        ossTemplate.query().previewObject(
                new JavaxOssPreviewContext(request, response),
