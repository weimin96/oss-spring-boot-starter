...
/**
 * Spring Boot 3.x OSS OpenAPI 自动配置。
 *
 ...
 * @author panwm
 */
@AutoConfiguration(after = OssAutoConfiguration3.class, before = OssWebAutoConfiguration3.class)
@ConditionalOnProperty(prefix = "oss", name = "enable", havingValue = "true")
public class OssOpenApiAutoConfiguration3 {

    private static final Logger log = LoggerFactory.getLogger(OssOpenApiAutoConfiguration3.class);
    ...
    public OpenApiOssController3 ossController(OssTemplate template) {
        log.info("Initializing OSS HTTP endpoint with OpenAPI metadata (Boot3)");
        return new OpenApiOssController3(template);
    }
}
