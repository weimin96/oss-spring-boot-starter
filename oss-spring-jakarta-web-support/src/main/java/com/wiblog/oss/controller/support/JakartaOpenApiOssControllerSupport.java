...
/**
 * `jakarta.servlet` 体系共享的 OpenAPI HTTP 端点实现。
 *
 * <p>Boot3 与 Boot4 使用同一套文档控制器实现，
 * 由版本 Starter 负责暴露各自公开类型。这样可以把 OpenAPI 路由、
 * 文档元数据和方法校验声明保持在同一处，降低后续升级成本。</p>
 *
 * @author panwm
 */
public abstract class JakartaOpenApiOssControllerSupport extends JakartaOssControllerSupport {

    public JakartaOpenApiOssControllerSupport(OssTemplate ossTemplate) {
        super(ossTemplate);
    }
