 ...
 * OssController4 单元测试。
 *
 * <p>Boot4 控制器的职责仍然是协议适配与委派，
 * 因此测试重点放在参数转换、路径提取和返回包装，确保升级到新框架版本后 HTTP 语义不漂移。</p>
 */
@DisplayName("OssController4 单元测试")
class OssController4UnitTest {
    ...
    private OssController4 controller;
    ...
    void setUp() {
        ...
        when(queryOperations.getOssProperties()).thenReturn(
                new OssProperties("http://localhost:9000", "ak", "sk", "minio", "unit-bucket"));

        controller = new OssController4(ossTemplate);
    }
